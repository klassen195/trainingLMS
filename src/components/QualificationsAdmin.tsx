...
export function CreateQualificationForm() {
  ...
  return (
    ...
    >
      <p className="text-sm font-medium">New qualification</p>
      <div className="space-y-2">
        <FieldLabel htmlFor="new-qualification-name">Name</FieldLabel>
        <Input
          id="new-qualification-name"
          required
          value={name}
          disabled={pending}
          placeholder="e.g. Aerial operator"
          onChange={(e) => setName(e.target.value)}
        />
      </div>
      {error ? <FieldError>{error}</FieldError> : null}
      <Button type="submit" size="sm" disabled={pending || !name.trim()}>
        {pending ? "Adding…" : "Add qualification"}
      </Button>
    </form>
  );
}

function SortableQualificationRow({ qualification }: { qualification: Qualification }) {
  const router = useRouter();
  const [editing, setEditing] = useState(false);
  const [name, setName] = useState(qualification.name);
  const [isActive, setIsActive] = useState(qualification.is_active);
  const [error, setError] = useState<string | null>(null);
  const [pending, startTransition] = useTransition();
  ...
  function reset() {
    ...
    setIsActive(qualification.is_active);
    setError(null);
    setEditing(false);
  }
      ...
      {!editing ? (
        <div className="flex items-center gap-2 px-2 py-1.5">
          <button
            ...
            aria-label={`Reorder ${qualification.name}`}
            {...attributes}
            {...listeners}
          >
          ...
          <div className="min-w-0 flex-1">
            <div className="flex min-w-0 items-center gap-2">
              <span className="truncate text-sm font-medium">{qualification.name}</span>
        ...
        >
          <div className="grid gap-2 sm:grid-cols-[1fr_auto] sm:items-end">
            <div className="space-y-1">
              <FieldLabel htmlFor={`qualification-name-${qualification.id}`}>Name</FieldLabel>
              <Input
                id={`qualification-name-${qualification.id}`}
                required
                value={name}
                disabled={pending}
                className="h-8"
          ...
          <div className="flex flex-wrap gap-2">
            ...
            <Button
              ...
              onClick={() => {
                if (
                  !window.confirm(
                    `Delete "${qualification.name}"? This cannot be undone if it is still in use.`
                  )
                ) {
                  return;
                }
...
export function QualificationsAdmin({
  qualifications,
}: {
  qualifications: Qualification[];
}) {
  const [ordered, setOrdered] = useState(qualifications);
  const [error, setError] = useState<string | null>(null);
  const [pending, startTransition] = useTransition();

  useEffect(() => {
    setOrdered(qualifications);
  }, [qualifications]);
  ...
  function handleDragEnd(event: DragEndEvent) {
    ...
    startTransition(async () => {
      try {
        await reorderQualifications({
          qualificationIds: next.map((row) => row.id),
        });
      } catch (err) {
        setOrdered(qualifications);
        setError(err instanceof Error ? err.message : "Failed to reorder qualifications");
      }
    });
  }
      ...
      <div className="space-y-2">
        {ordered.length === 0 ? (
          <p className="rounded-md border border-dashed px-4 py-8 text-center text-sm text-muted-foreground">
            No qualifications yet. Add one on the right.
          </p>
        ) : (
          <>
            ...
            >
              ...
              >
                <ul className="overflow-hidden rounded-md border">
                  {ordered.map((row) => (
                    <SortableQualificationRow key={row.id} qualification={row} />
                  ))}
                </ul>
              </SortableContext>
            </DndContext>
