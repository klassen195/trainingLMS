...
} from "@/app/admin/training-categories/actions";
...
export function CreateTrainingCategoryForm() {
  ...
  return (
    ...
    >
      ...
      <div className="space-y-2">
        <FieldLabel htmlFor="new-training-category-name">Name</FieldLabel>
        <Input
          id="new-training-category-name"
          required
          value={name}
          disabled={pending}
...
function SortableCategoryRow({ category }: { category: TrainingCategory }) {
  const router = useRouter();
      ...
      {!editing ? (
        ...
        <form
          ...
          onSubmit={(event) => {
            ...
            startTransition(async () => {
              try {
                await updateTrainingCategory(category.id, {
                  name,
        ...
        >
          <div className="grid gap-2 sm:grid-cols-[1fr_auto] sm:items-end">
            <div className="space-y-1">
              <FieldLabel htmlFor={`training-category-name-${category.id}`}>Name</FieldLabel>
              <Input
                id={`training-category-name-${category.id}`}
                required
                value={name}
                disabled={pending}
                className="h-8"
          ...
          <div className="flex flex-wrap gap-2">
            ...
            <Button
              ...
              onClick={() => {
                if (
                  !window.confirm(
                    `Delete "${category.name}"? This cannot be undone if sessions still use it.`
                  )
                ) {
                  return;
                }
...
export function TrainingCategoriesAdmin({
  categories,
}: {
  ...
  function handleDragEnd(event: DragEndEvent) {
    ...
    startTransition(async () => {
      try {
        await reorderTrainingCategories({
